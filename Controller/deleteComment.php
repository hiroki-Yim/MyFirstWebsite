...
$num = requestValue("commentNum");       //현재 보고있는 글의 번호를 들고옴
...
if($sid == $writer){              //세션 아이디(현재 접속자)와 쓰여진 글의 작성자가 같은지 확인함.
  if($num){                       //글이 존재하면
  $bdao->deleteComment($num);         //bdo에서 만든 delete문을 실행함
  okGo("해당 댓글이 삭제되었습니다.", "../View/kakaoView/views.php?num=$board_num&page=".$page);
  }else{
    errorBack("이미 삭제된 댓글 입니다.", "../View/kakaoView/board.php"); //글이 존재하지 않으면
  }
}else{
